...
- `f` - информации о процессах в виде дерева процессовю
...
3. `tail -f ~/test.out`
4. приостанавливает выполнение `Ctrl+z` | `bg` background - чтобы запустить в фоновом режиме  | `fg` foreground - продолжить выполнение приостановленного процесса
5. 
```bash
...
```
- `/dev/stderr` - это символьная ссылка на файловый дескриптор, используемый для записи сообщений об ошибках.
